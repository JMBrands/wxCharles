...
const vector<Track> TEST_DB = {
    ...
    ,{"TestA",                          "TestA",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"TestA",                          "TestB",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"TestB",                          "TestA",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"TestB",                          "TestB",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"Test",                           "TestC",                       2022,1, "TestB",                       "test",                              { 2,42},"Test"}
    ,{"Test",                           "TestC",                       2022,2, "testA",                       "test",                              { 2,42},"Test"}
    ,{"Test",                           "TestE",                       2022,1, "Testc",                       "test",                              { 2,42},"Test"}
    ,{"test",                           "TestD",                       2022,2, "testc",                       "test",                              { 2,42},"Test"}
...
const vector<Track> TEST_DB_SORTED_TIME = {
     {"Colin Stetson and Sarah Neufeld","Never were the way she was",  2015,8, "Flight",                      "jazz,instrumental,experimental",    { 1,38},"U.S.A.,Canada"}
    ,{"TestA",                          "TestA",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"TestA",                          "TestB",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
    ,{"TestB",                          "TestA",                       2022,1, "Test",                        "test",                              { 1,42},"Test"}
...
const vector<Track> TEST_DB_SORTED_TITLE = {
     {"Colin Stetson and Sarah Neufeld","Never were the way she was",  2015,4, "And still they move",         "jazz,instrumental,experimental",    { 2,56},"U.S.A.,Canada"}
    ,{"Adele",                          "19",                          2008,2, "Best for last",               "pop,debut album",                   { 4,18},"England"}
    ,{"All them witches",               "Dying surfer meets his maker",2015,9, "Blood and sand / milk and endless waters","alternative,rock",      { 7,20},"U.S.A."}
    ,{"All them witches",               "Dying surfer meets his maker",2015,1, "Call me star",                "alternative,rock",                  { 3,54},"U.S.A."}
...
TEST(part1, track_less_than1)
{
    EXPECT_LT(TEST_DB[4], TEST_DB[1]);
}

TEST(part1, track_less_than2)
{
    EXPECT_LT(TEST_DB[11], TEST_DB[12]);
}

TEST(part1, track_less_than3)
{
...
TEST(sorting_extra, is_sorted)
{
    ...
    EXPECT_TRUE(is_sorted(TRACKS, make_slice(0, 1), less<Track>()));
    EXPECT_FALSE(is_sorted(TRACKS, make_slice(0, 2), less<Track>()));
    EXPECT_TRUE(is_sorted(TRACKS, make_slice(1, 5), less<Track>()));
    EXPECT_FALSE(is_sorted(TRACKS, make_slice(1, 6), less<Track>()));
}
...
TEST(sorting_extra, insert1)
{
    ...
    insert(actual, make_slice(0, 4), less<Track>());
...
TEST(sorting_extra, max_value_at)
{
    ...
    EXPECT_EQ(max_value_at(TRACKS, make_slice(TRACKS), less<Track>()), EXPECTED);
}
...
TEST(sorting_extra, bubble)
{
    ...
    EXPECT_TRUE(bubble(actual, make_slice(actual), less<Track>()));
    EXPECT_EQ(actual, EXPECTED);
    EXPECT_TRUE(bubble(actual, make_slice(actual), less<Track>()));
}
...
TEST(sorting_extra, bubble_noswap)
{
    ...
    EXPECT_FALSE(bubble(actual, make_slice(actual), less<Track>()));
    EXPECT_EQ(actual, EXPECTED);
}
...
TEST(sorting_extra, is_a_heap)
{
    ...
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 1), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 2), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 3), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 4), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 5), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP1, make_slice(0, 6), less<Track>()));
    EXPECT_FALSE(is_a_heap(HEAP1, make_slice(0, 7), less<Track>()));
    EXPECT_TRUE(is_a_heap(HEAP2, make_slice(0, 3), less<Track>()));
    EXPECT_FALSE(is_a_heap(HEAP2, make_slice(0, 4), less<Track>()));
    EXPECT_FALSE(is_a_heap(HEAP2, make_slice(0, 5), less<Track>()));
    EXPECT_FALSE(is_a_heap(HEAP2, make_slice(0, 6), less<Track>()));
    EXPECT_FALSE(is_a_heap(HEAP2, make_slice(0, 7), less<Track>()));
}
...
TEST(sorting_extra, build_heap)
{
    ...
    build_heap(tracks, less<Track>());
    EXPECT_TRUE(is_a_heap(tracks, make_slice(tracks), less<Track>()));
}
...
TEST(sorting_extra, largest_child)
{
    ...
    EXPECT_FALSE(largest_child(HEAP, 0, 0, child, which, less<Track>()));
    EXPECT_FALSE(largest_child(HEAP, 2, 4, child, which, less<Track>()));
    EXPECT_TRUE(largest_child(HEAP, 0, 1, child, which, less<Track>()));
    EXPECT_EQ(which, 1);
    EXPECT_TRUE(largest_child(HEAP, 0, 2, child, which, less<Track>()));
    EXPECT_EQ(which, 2);
    EXPECT_TRUE(largest_child(HEAP, 1, 3, child, which, less<Track>()));
    EXPECT_EQ(which, 3);
    EXPECT_TRUE(largest_child(HEAP, 1, 4, child, which, less<Track>()));
    EXPECT_EQ(which, 3);
}
...
TEST(sorting_extra, pick_heap)
{
    ...
    pick_heap(actual, less<Track>());
    EXPECT_EQ(actual, EXPECTED);
}

TEST(part1, insertion_sort)
{
    vector<Track> database = TEST_DB;
    insertion_sort(database, less<Track>());
    debug_database_difference(database, TEST_DB_SORTED);
    EXPECT_EQ(TEST_DB_SORTED, database);
}

TEST(part1, selection_sort)
{
    vector<Track> database = TEST_DB;
    selection_sort(database, less<Track>());
    debug_database_difference(database, TEST_DB_SORTED);
    EXPECT_EQ(TEST_DB_SORTED, database);
}

TEST(part1, bubble_sort)
{
    vector<Track> database = TEST_DB;
    bubble_sort(database, less<Track>());
    debug_database_difference(database, TEST_DB_SORTED);
    EXPECT_EQ(TEST_DB_SORTED, database);
}

TEST(part1, heap_sort)
{
    vector<Track> database = TEST_DB;
    heap_sort(database, less<Track>());
    debug_database_difference(database, TEST_DB_SORTED);
    EXPECT_EQ(TEST_DB_SORTED, database);
}

TEST(part2, track_less_than1)
{
    EXPECT_TRUE(less_than_time(TEST_DB[4], TEST_DB[2]));
}

TEST(part2, track_less_than2)
{
    EXPECT_TRUE(less_than_time(TEST_DB[1], TEST_DB[0]));
}

TEST(part2, track_less_than3)
{
    EXPECT_TRUE(less_than_time(TEST_DB[40], TEST_DB[41]));
}

TEST(part2, track_less_than4)
{
    EXPECT_TRUE(less_than_time(TEST_DB[40], TEST_DB[42]));
}

TEST(part2, insertion_sort)
{
    vector<Track> database = TEST_DB;
    insertion_sort(database, less_than_time);
    debug_database_difference(database, TEST_DB_SORTED_TIME);
    EXPECT_EQ(TEST_DB_SORTED_TIME, database);
}

TEST(part2, selection_sort)
{
    vector<Track> database = TEST_DB;
    selection_sort(database, less_than_time);
    debug_database_difference(database, TEST_DB_SORTED_TIME);
    EXPECT_EQ(TEST_DB_SORTED_TIME, database);
}

TEST(part2, bubble_sort)
{
    vector<Track> database = TEST_DB;
    bubble_sort(database, less_than_time);
    debug_database_difference(database, TEST_DB_SORTED_TIME);
    EXPECT_EQ(TEST_DB_SORTED_TIME, database);
}

TEST(part2, heap_sort)
{
    vector<Track> database = TEST_DB;
    heap_sort(database, less_than_time);
    debug_database_difference(database, TEST_DB_SORTED_TIME);
    EXPECT_EQ(TEST_DB_SORTED_TIME, database);
}

TEST(running_time, length_less_than)
{
    EXPECT_LT((Length{ 4,  4}), (Length{22,  6}));
    EXPECT_LT((Length{ 9, 32}), (Length{27, 33}));
    EXPECT_LT((Length{27, 48}), (Length{50, 46}));
    EXPECT_LT((Length{37, 49}), (Length{54, 49}));
    EXPECT_LE((Length{57, 54}), (Length{57, 54}));
    EXPECT_GE((Length{57, 54}), (Length{57, 54}));
    EXPECT_GT((Length{57, 55}), (Length{57, 54}));
}

TEST(part3, track_less_than1)
{
    EXPECT_TRUE(less_than_title(TEST_DB[45], TEST_DB[44]));
}

TEST(part3, track_less_than2)
{
    EXPECT_TRUE(less_than_title(TEST_DB[44], TEST_DB[47]));
}

TEST(part3, track_less_than3)
{
    EXPECT_TRUE(less_than_title(TEST_DB[47], TEST_DB[46]));
}

TEST(part3, track_less_than4)
{
    EXPECT_TRUE(less_than_title(TEST_DB[44], TEST_DB[48]));
}

TEST(part3, insertion_sort)
{
    vector<Track> database = TEST_DB;
    insertion_sort(database, less_than_title);
    debug_database_difference(database, TEST_DB_SORTED_TITLE);
    EXPECT_EQ(TEST_DB_SORTED_TITLE, database);
}

TEST(part3, selection_sort)
{
    vector<Track> database = TEST_DB;
    selection_sort(database, less_than_title);
    debug_database_difference(database, TEST_DB_SORTED_TITLE);
    EXPECT_EQ(TEST_DB_SORTED_TITLE, database);
}

TEST(part3, bubble_sort)
{
    vector<Track> database = TEST_DB;
    bubble_sort(database, less_than_title);
    debug_database_difference(database, TEST_DB_SORTED_TITLE);
    EXPECT_EQ(TEST_DB_SORTED_TITLE, database);
}

TEST(part3, heap_sort)
{
    vector<Track> database = TEST_DB;
    heap_sort(database, less_than_title);
    debug_database_difference(database, TEST_DB_SORTED_TITLE);
    EXPECT_EQ(TEST_DB_SORTED_TITLE, database);
}
