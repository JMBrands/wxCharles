...
const string HYPHEN_MADDNESS_FILENAME = "hyphen_madness.txt";
const int NO_OF_WORDS_HYPHEN_MADDNESS = 9;
string content_hyphen_madness [NO_OF_WORDS_HYPHEN_MADDNESS] =
  {"one", "two", "thre-e", "four", "five", "six", "seven", "eight", "nine"};
...
TEST(enter_command, hyphen_maddness)
{
    string content[MAX_NO_OF_WORDS];
    EXPECT_EQ(enter_command(HYPHEN_MADDNESS_FILENAME, content), NO_OF_WORDS_HYPHEN_MADDNESS);
    EXPECT_TRUE(equal_content(content, content_hyphen_madness, NO_OF_WORDS_HYPHEN_MADDNESS));
}
